...
class DischargeRAGPipeline:
    ...
    def route_query(self, text: str) -> dict:
        """
        Agentic router — uses classify_intent to decide which sections
        to retrieve for, rather than always running all 4 retrievals.
        
        This is the core agentic behavior: the system reasons about
        the input and selects the minimal set of tools needed.
        """
        from .preprocess import classify_intent
        classification = classify_intent(text)
        intent = classification["top_intent"]

        # Define which retrieval sections each intent needs
        INTENT_TO_SECTIONS = {
            "medication":    ["medications"],
            "follow_up":     ["follow_up"],
            "warning_sign":  ["warning_signs"],
            "diagnosis":     ["diagnosis"],
            "general":       ["diagnosis", "medications", "follow_up", "warning_signs"],
        }

        # Full discharge notes always get all 4 sections
        is_full_note = len(text.split()) > 15 or "discharge diagnosis" in text.lower()

        sections = INTENT_TO_SECTIONS["general"] if is_full_note else INTENT_TO_SECTIONS.get(intent, INTENT_TO_SECTIONS["general"])

        return {
            "intent": intent,
            "scores": classification["scores"],
            "is_full_note": is_full_note,
            "sections_to_retrieve": sections,
            "reasoning": f"Detected intent '{intent}' → retrieving sections: {sections}",
        }
