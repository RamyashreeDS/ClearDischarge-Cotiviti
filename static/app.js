      ...
      for (const { event, data } of events) {
        switch (event) {
          ...
          case 'sources':
            ...
            break;
          case 'routing': {
            if (!isDischarge) break;
            const routingPanel = aiEl.querySelector('.routing-panel');
            if (routingPanel) {
              routingPanel.style.display = 'block';
              routingPanel.innerHTML = `
                <div style="margin-top:12px; padding:10px 14px; border-radius:8px; background:rgba(99,102,241,0.1); border:1px solid rgba(99,102,241,0.3); font-size:13px;">
                  <div style="font-weight:600; color:#818cf8; margin-bottom:4px;">🤖 Agentic Router</div>
                  <div style="color:var(--text-secondary);">${data.reasoning}</div>
                </div>`;
            }
            break;
          }
          ...
          case 'risk':
            if (!isDischarge) break;
            const riskPanel = aiEl.querySelector('.risk-panel');
            if (riskPanel) {
              const riskColor = data.level === 'HIGH' ? '#f87171' : data.level === 'MEDIUM' ? '#fb923c' : '#4ade80';
              riskPanel.style.display = 'block';
              riskPanel.innerHTML = `
                <div style="margin-top:8px; padding:10px 14px; border-radius:8px; background:rgba(0,0,0,0.2); border:1px solid ${riskColor}40; font-size:13px;">
                  <div style="font-weight:600; color:${riskColor}; margin-bottom:4px;">⚕️ 30-Day Readmission Risk: ${data.level}</div>
                  <div style="color:var(--text-secondary); margin-bottom:4px;">${data.explanation}</div>
                  <div style="color:var(--text-secondary); font-style:italic;">${data.recommendation}</div>
                </div>`;
            }
            break;
          case 'token':
