...
def _extract_text(content: bytes, filename: str) -> str:
    ...
    if ext in (".doc",):
        raise HTTPException(422, "Legacy .doc format is not supported.")
    ...
    if ext in (".png", ".jpg", ".jpeg", ".bmp", ".tiff", ".tif", ".webp", ".gif"):
        try:
            ...
            if not text:
                raise HTTPException(422, "No text could be read from the image.")
            return text
...
async def explain(req: ExplainRequest):
    ...
    async def generate() -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # ── Agentic routing + risk scoring (once only) ────────────────────
            routing = p.route_query(req.note_text)
            risk = score_readmission_risk(req.note_text)
            yield sse("routing", {
                "intent": routing["intent"],
                "sections": routing["sections_to_retrieve"],
            ...
            # ── Always use reliable fallback template for discharge notes ─────
...
async def vitals(req: VitalsRequest):
    from discharge_rag.preprocess import detect_vitals_anomaly
    readings = {
        "weight_kg": req.weight_kg,
        "systolic_bp": req.systolic_bp,
        "diastolic_bp": req.diastolic_bp,
    }
    result = detect_vitals_anomaly(readings)
    return result
